...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6">
            ...
            <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-5 mb-4">
                <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-1">
                        ...
                        @endfor
                        <span class="ml-2 text-sm text-slate-500">({{ $myReview->rating }}/5)</span>
                    </div>
                    <span class="text-xs text-slate-400">{{ $myReview->updated_at->diffForHumans() }}</span>
                </div>
                <p class="text-slate-700">{{ $myReview->comment }}</p>
            </div>
        ...
        <div id="review-form" class="{{ $myReview ? 'hidden mt-5' : '' }}">
            <form method="POST" action="{{ route('reviews.store') }}" class="space-y-5">
                    ...
                    <div class="flex items-center gap-2" id="star-picker">
                        @for($i = 1; $i <= 5; $i++)
                            <button type="button" data-value="{{ $i }}"
                                    class="star-btn w-9 h-9 text-2xl transition-transform hover:scale-110 focus:outline-none"
                                    onclick="setRating({{ $i }})">
                                ☆
                            </button>
                        @endfor
                ...
                <div>
                    <label for="comment" class="block text-sm font-medium text-slate-700 mb-1">Comment</label>
                    <textarea name="comment" id="comment" rows="4"
                              class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none"
                              placeholder="Share your experience with TaskFlow...">{{ old('comment', $myReview?->comment ?? '') }}</textarea>
                    @error('comment')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <button type="submit"
                        class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold transition-colors shadow-sm">
                    {{ __('Save Review') }}
                </button>
            </form>
        </div>
    ...
    {{-- All Reviews --}}
    <div>
        <h3 class="text-lg font-bold text-slate-800 mb-4">Community Reviews</h3>
...
function setRating(value) {
    ...
    btns.forEach((btn, idx) => {
        ...
        btn.style.color = idx < value ? '#facc15' : '#94a3b8';
    });
}
